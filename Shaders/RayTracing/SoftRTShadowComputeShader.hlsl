...
void SoftShadow(inout RWTexture2D<float> RTShadow, uint2 PixelCoord, float2 UV, float MipRate, float OpaqueDepth, bool bIsTranslucent)
{
    ...
    Penumbra = lerp(Penumbra, 0.0f, pow(MipWeight, 0.5f));

	// actually sampling
	float Atten = 0.0f;
    float DepthDiffThres = lerp(0.001f, 0.0001f, MipWeight);
	...
	for (int I = -PCSS_INNERLOOP; I <= PCSS_INNERLOOP; I++)
	{
		...
		{
            float2 ShadowUV = UV + float2(I, J) * Penumbra * UHShadowResolution.zw;
			
            float CmpDepth = (bIsTranslucent) ? TranslucentTexture.SampleLevel(LinearClampped, ShadowUV, 0).r : DepthTexture.SampleLevel(LinearClampped, ShadowUV, 0).r;
...
void SoftRTShadowCS(uint3 DTid : SV_DispatchThreadID)
{
    ...
    float2 UV = float2(PixelCoord + 0.5f) * UHShadowResolution.zw;
