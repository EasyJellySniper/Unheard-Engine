...
// BEGIN UHE MOD - expose this function in imgui header
...
void ImGui_ImplVulkanH_CreateWindowSwapChain(VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, const VkAllocationCallbacks* allocator, int w, int h, uint32_t min_image_count)
{
    ...
    if (wd->UseDynamicRendering == false)
    {
        ...
        //ImGui_ImplVulkan_CreatePipeline(device, allocator, nullptr, wd->RenderPass, VK_SAMPLE_COUNT_1_BIT, &wd->Pipeline, bd->Subpass);
    }
...
void ImGui_ImplVulkanH_CreateOrResizeWindow(VkInstance instance, VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, uint32_t queue_family, const VkAllocationCallbacks* allocator, int width, int height, uint32_t min_image_count)
{
    ...
    //ImGui_ImplVulkan_CreatePipeline(device, allocator, nullptr, wd->RenderPass, VK_SAMPLE_COUNT_1_BIT, &wd->Pipeline, g_VulkanInitInfo.Subpass);
...
static void ImGui_ImplVulkan_CreateWindow(ImGuiViewport* viewport)
{
    ...
    // BEGIN UHE MOD - select the surface format based on swapchain format instead
    wd->SurfaceFormat = ImGui_ImplVulkanH_SelectSurfaceFormat(v->PhysicalDevice, wd->Surface, &v->SwapChainFormat, 1, v->SwapChainColorSpace);
    // END UHE MOD
...
// BEGIN UHE MOD - pass pipeline data
static void ImGui_ImplVulkan_RenderWindow(ImGuiViewport* viewport, void* InCustomData)
// END UHE MOD
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_ViewportData* vd = (ImGui_ImplVulkan_ViewportData*)viewport->RendererUserData;
    ...
    // BEGIN UHE MOD - use custom data instead
    ImGui_Vulkan_CustomData* CustomData = (ImGui_Vulkan_CustomData*)InCustomData;

    ImGui_ImplVulkanH_Frame* fd = &wd->Frames[wd->FrameIndex];
    ImGui_ImplVulkanH_FrameSemaphores* fsd = &wd->FrameSemaphores[wd->SemaphoreIndex];
    {
        {
          err = vkAcquireNextImageKHR(v->Device, wd->Swapchain, UINT64_MAX, fsd->ImageAcquiredSemaphore, nullptr, &wd->FrameIndex);
          check_vk_result(err);
    ...
    ImGui_ImplVulkan_RenderDrawData(viewport->DrawData, fd->CommandBuffer, (CustomData) ? CustomData->Pipeline : wd->Pipeline);
    // END UHE MOD
